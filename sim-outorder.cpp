...
using namespace std;
extern bool takenFlag;
extern unsigned int				/* latency of access in cycles */
...
/* smt_qos options********/
/* # of branches to track - K */
static unsigned int smt_qos_branch_depth;
/* Max # of branches miss to allow before throttling - N */
static unsigned int smt_qos_branch_mdepth;
/* Throttle % of IQ for misbehaving contxts/Threads */
static unsigned int smt_qos_th_iq_pct;
static unsigned int smt_qos_th_rf_pct;
static unsigned int smt_qos_th_spec_depth;
static int smt_qos_enable;
int alg1;
int brupdate;
int brupdate_cond;
int brhistory;
static int smt_qos_enable_iq;
static int smt_qos_enable_rf;
static int smt_qos_enable_spec;
/**************/

/*Flag to check if in retire or not */
...
/*
 * SMT QOS Idea State Tracking & Implementation
 */
enum smt_qos_state {
	SMTQ_NORMAL = 0,
	SMTQ_THROTTLE
...
class smtq_br_info {
public:
	smtq_br_info(INST_SEQ_TYPE pseq, unsigned long long pcnt):
			seq(pseq)
			,cnt(pcnt)
			,clear(false)
		{}
...
public:
	SMT_QOS()
		...
		,total_rf_int_size(0)
		,total_spec_depth(0)
		,total_squashed_inst(0)
		,number_squashes(0)
	{
	}
	deque<bool> bp;
	deque<smtq_br_info> spec_br;
	smt_qos_state state;
	void update(bool hit) {
		...
		if (bp.size() > smt_qos_branch_depth) {
			bp.pop_front();
		}
		if (bp.size() >= smt_qos_branch_mdepth) {
			unsigned int pmiss = 0;
			for (unsigned int i=0; i<bp.size(); i++) {
				if (false == bp[i]) {
					pmiss++;
				}
			}
			if (pmiss >= smt_qos_branch_mdepth) {
				state = SMTQ_THROTTLE;
			} else {
				state = SMTQ_NORMAL;
			}
	...
	bool is_throttled(){
		return (spec_br.size() && (state==SMTQ_THROTTLE));
	}
	...
	void update_stats() {
		...
		unsigned int sd = get_spec_depth();
		if (sd > max_spec_depth) {
			max_spec_depth = sd;
		}
		total_spec_depth += sd;

	}
	void print_stats(unsigned int c) {
		cout << "TH_CYCLES[" << c << "]: " << th_cycles << " out of  " << sim_cycle << ", pct " << (double)(100.0*(double)th_cycles/(double)sim_cycle) << endl;
		cout << "BP_MISS[" << c << "]: " << bmiss << " out of  " << total_br << ", pct " << (double)(100.0*(double)bmiss/(double)total_br) << endl;
		cout << "IQ_THs[" << c << "]: " << iq_ths << endl;
		cout << "RF_THs[" << c << "]: " << rf_ths << endl;
		cout << "SPEC_BR[" << c << "]:" << spec_br.size() << endl;
		cout << "SPEC_DEPTH[" << c << "]:" << get_spec_depth() << endl;
		cout << "MAX_SPEC_DEPTH[" << c << "]: " << max_spec_depth << endl;
		cout << "AVG_SPEC_DEPTH[" << c << "]: " << (double)total_spec_depth/(double)sim_cycle << endl;
		cout << "AVG_IQ_SIZE[" << c << "]: " << (double)total_iq_size/(double)sim_cycle << endl;
		cout << "AVG_RF_INT_SIZE[" << c << "]: " << (double)total_rf_int_size/(double)sim_cycle << endl;
		cout << "AVG_RF_FLT_SIZE[" << c << "]: " << (double)total_rf_flt_size/(double)sim_cycle << endl;
		cout << "TH_SPEC_CYCLES[" << c << "]: " << th_spec_cycles<< endl;
		cout << "TH_SPEC_CYCLES_PCT[" << c << "]: " <<  (double)(100.0*(double)th_spec_cycles/(double)sim_cycle)<<endl;
		cout << "AVG_SQUASH[" << c << "]: " <<  (double)total_squashed_inst/(double)number_squashes<<endl;
		cout << "total squash[" << c << "]: " <<  total_squashed_inst<<endl;
		cout << "number of squashes[" << c << "]: " <<  number_squashes<<endl;
	}
	void clear_br(INST_SEQ_TYPE seq, bool erase_only=false) {
		bool found = false;
		for (deque<smtq_br_info>::iterator itr = spec_br.begin(); itr != spec_br.end(); itr++) {
			if ((itr->seq == seq) || found) {
				found = true;
				if (erase_only) {
					itr->clear = true;
					spec_br.erase(itr);
	...
	unsigned int get_spec_depth() {
		unsigned int cnt = spec_br.size();
		for (deque<smtq_br_info>::iterator itr = spec_br.begin(); itr != spec_br.end(); itr++) {
			cnt += itr->cnt;
	...
	unsigned int get_spec_br() {
		return spec_br.size();
	}
	/* Stats */
	unsigned long long th_cycles;
	unsigned long long bmiss;
	unsigned long long total_br;
	unsigned long long iq_ths;
	unsigned long long rf_ths;
	unsigned long long max_spec_depth;

	unsigned long long total_spec_depth;
	unsigned long long th_spec_cycles;
	unsigned long long avg_iq_size;
	unsigned long long total_iq_size;
	unsigned long long total_rf_int_size;
	unsigned long long total_rf_flt_size;
	unsigned long long total_squashed_inst;
	unsigned long long number_squashes;

};

SMT_QOS smtq[MAX_CONTEXTS];


void print_smtq_stats(unsigned int numc) {
	cout << " ----- SMTQ STATS ----- " << endl;
	cout<<"THROUGHPUT IPC: "<<sim_num_insn / (float)sim_cycle<<endl;
	cout << "SMTQ Enable: " << smt_qos_enable << ", IQ " << smt_qos_enable_iq << ". RF " << smt_qos_enable_rf <<". SPEC " << smt_qos_enable_spec << endl;
	for (unsigned int i=0; i<numc; i++) {
		smtq[i].print_stats(i);
	}
}
...
static int find_free_physreg(enum reg_type type, unsigned int cid){
	unsigned long long temp_flt_rf_size;
	unsigned long long temp_int_rf_size;
	if(type == REG_INT){
		unsigned int cid_cnt = 0;
		for (unsigned int i=0; i<rf_size;i++){
			if ((int_reg_file[i].cid==cid) && int_reg_file[i].state!=REG_FREE)
				cid_cnt++;

		}
		smtq[cid].total_rf_int_size+=cid_cnt;

		if (smt_qos_enable && smt_qos_enable_rf && (num_contexts>1) && (smtq[cid].is_throttled())) {
			double pct = (double) cid_cnt*100.0/(double)rf_size;
			if (pct>=smt_qos_th_rf_pct){
				smtq[cid].rf_ths++;
				return -1;
			}
		}
		...
		unsigned int cid_cnt = 0;
		for (unsigned int i=0; i<rf_size;i++){
			if ((fp_reg_file[i].cid==cid) && fp_reg_file[i].state!=REG_FREE)
				cid_cnt++;
		}
		smtq[cid].total_rf_flt_size+=cid_cnt;
		if (smt_qos_enable && smt_qos_enable_rf && (num_contexts>1) && (smtq[cid].is_throttled())){
			double pct = (double) cid_cnt*100.0/(double)rf_size;
			if (pct>=smt_qos_th_rf_pct){
				smtq[cid].rf_ths++;
...
/* actual IQ */
class iq_entry_class {
public:
	iq_entry_class():state(IQ_ENTRY_FREE)
		,cid(0)
	{}
	iq_entry_state state;
	unsigned int cid;
};
...
static void free_iq_entry(int entry_num){
	...
	assert(iq[entry_num].state == IQ_ENTRY_ALLOC);
	iq[entry_num].state = IQ_ENTRY_FREE;
}
...
static int find_iq_entry(unsigned int cid){
	int i;

	unsigned int cid_cnt = 0;
	for(i=0;i<iq_size;i++){
		if((iq[i].state == IQ_ENTRY_ALLOC) && (iq[i].cid == cid)){
			cid_cnt++;
		}
	}
	smtq[cid].total_iq_size+=cid_cnt;

	if (smt_qos_enable && smt_qos_enable_iq && (num_contexts > 1) && (smtq[cid].is_throttled())) {
		double pct = (double)cid_cnt*100.0/(double)iq_size;
		if (pct >= smt_qos_th_iq_pct) {
			smtq[cid].iq_ths++;
			return -1;
		}
	}
 ...
 */
static int alloc_iq_entry(unsigned int cid){
	/* find the location of a free IQ entry */
	int i = find_iq_entry(cid);
	...
	assert(iq[i].state == IQ_ENTRY_FREE);
	iq[i].state = IQ_ENTRY_ALLOC;
	iq[i].cid = cid;
...
sim_reg_options(struct opt_odb_t *odb)
{
	...
	opt_reg_uint(odb, "-max:inst", "maximum number of inst's to execute",
			&max_insts, /* default */10000000,
			/* print */TRUE, /* format */NULL);
	// SMT_QOS
	opt_reg_flag(odb, "-smt_qos::enable", "SMT QOS Enable SMTQ Feature",
			&smt_qos_enable, /* default */FALSE,
			/* print */TRUE, /* format */NULL);

	opt_reg_flag(odb, "-alg1::enable", "AJDSOAIJDASOI",
			&alg1, /* default */FALSE,
			/* print */TRUE, /* format */NULL);

	opt_reg_flag(odb, "-alg1::brupdate", "AJDSOAIJDASOI",
				&brupdate, /* default */TRUE,
				/* print */TRUE, /* format */NULL);

	opt_reg_flag(odb, "-alg1::brupdate_cond", "AJDSOAIJDASOI",
				&brupdate_cond, /* default */FALSE,
				/* print */TRUE, /* format */NULL);

	opt_reg_flag(odb, "-alg1::brhistory", "AJDSOAIJDASOI",
				&brhistory, /* default */TRUE,
				/* print */TRUE, /* format */NULL);

	opt_reg_flag(odb, "-smt_qos::enable_rf", "SMT QOS Enable SMTQ Feature for RF",
				&smt_qos_enable_rf, /* default */FALSE,
				/* print */TRUE, /* format */NULL);

	opt_reg_flag(odb, "-smt_qos::enable_iq", "SMT QOS Enable SMTQ Feature for IQ",
			&smt_qos_enable_iq, /* default */FALSE,
			/* print */TRUE, /* format */NULL);

	opt_reg_flag(odb, "-smt_qos::enable_spec", "SMT QOS Enable SMTQ Feature for SPEC",
				&smt_qos_enable_spec, /* default */TRUE,
				/* print */TRUE, /* format */NULL);

	opt_reg_uint(odb, "-smt_qos::branch_depth", "SMT QOS Branch History Depth - K",
			&smt_qos_branch_depth, /* default */10,
			/* print */TRUE, /* format */NULL);
	opt_reg_uint(odb, "-smt_qos::branch_mdepth", "SMT QOS Branch History Depth - N",
			&smt_qos_branch_mdepth, /* default */4,
			/* print */TRUE, /* format */NULL);
	opt_reg_uint(odb, "-smt_qos::th_iq_pct", "SMT QOS IQ Throttle Limit",
			&smt_qos_th_iq_pct, /* default */40,
			/* print */TRUE, /* format */NULL);

	opt_reg_uint(odb, "-smt_qos::th_rf_pct", "SMT QOS RF Throttle Limit",
				&smt_qos_th_rf_pct, /* default */40,
				/* print */TRUE, /* format */NULL);

	opt_reg_uint(odb, "-smt_qos::th_spec_depth", "SMT QOS Speculation Depth Throttle Limit",
					&smt_qos_th_spec_depth, /* default */5,
					/* print */TRUE, /* format */NULL);
	...
	opt_reg_string(odb, "-fetch:policy",
			"fetch policy",
			&fetch_policy, /* default */"round_robin",
			/* print */TRUE, /* format */NULL);
		...
		int argc, char **argv)        /* command line arguments */
{
	...
	for(i = 0; i < MAX_CONTEXTS;i++)
	{
		...
		if (i==0)
			contexts[i].pred->mypred = new my_predictor();
		else
			contexts[i].pred->mypred = NULL;
		std::cout << " =============================================== creating " << i << "MY_PRED" << std::endl;
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
	...
	for(i = 0; i < num_contexts; i++){
		if (contexts[i].load_lat_pred){
			...
			/* get a name for this predictor */
			switch (contexts[i].load_lat_pred->pclass)
			{
			case BPredComb:
				name = "cpred_comb";
				break;
		...
		char **envp)		/* program environment */
{
	...
	/* initalize the LSQ for this context */
...
static int alloc_physreg(struct ROB_entry* rob_entry){
	...
	/* find a new physreg */
	rob_entry->physreg = find_free_physreg(my_regs.dest, rob_entry->context_id);
	...
	if(my_regs.dest == REG_INT){
		...
		int_reg_file[rob_entry->physreg].cid = rob_entry->context_id;
		...
		fp_reg_file[rob_entry->physreg].cid = rob_entry->context_id;
...
{
	contexts[context_id].ROB = (ROB_entry *)calloc(ROB_size, sizeof(struct ROB_entry));
...
lsq_init(int context_id)
{
	...
	contexts[context_id].LSQ = (ROB_entry *)calloc(LSQ_size, sizeof(struct ROB_entry));
	...
	while (contexts[context_id].ROB_num > 0 && committed < commit_width)
	{
		...
		if (!rs->completed)
		{
			if((last_commit_cycle + COMMIT_TIMEOUT) <= sim_cycle){
				...
				fprintf(stderr, "\n %d %d\n", rs->dispatched, find_iq_entry(context_id));
			}
			break;
		}
				...
				&& (MD_OP_FLAGS(rs->op) & F_CTRL))
		{
			...
			bpred_update(false,contexts[context_id].pred,
			...
			assert(context_id == rs->context_id);
			bool correct_pred = (rs->pred_PC == rs->next_PC);
			smtq[rs->context_id].update(correct_pred);
			if (!rs->recover_inst && !correct_pred)
				smtq[rs->context_id].clear_br(rs->seq);
		}
...
{
	smtq[context_id].number_squashes++;
	...
	{
		smtq[context_id].total_squashed_inst++;
		...
		if(contexts[context_id].ROB[ROB_index].in_IQ >= 1){
			...
			contexts[context_id].debug_count--;
			assert(contexts[context_id].debug_count >= 0);
		}
...
writeback(void)
{
	...
	while ((rs = eventq_next_event()))
	{
				...
				&& (MD_OP_FLAGS(rs->op) & F_CTRL))
		{
			...
			bpred_update(!brupdate, contexts[rs->context_id].pred,
					/* branch address */rs->PC,
					/* actual target address */rs->next_PC,
					/* taken? */rs->next_PC != (rs->PC +
							sizeof(md_inst_t)),
		...
		if (contexts[rs->context_id].pred
				...
				&& (MD_OP_FLAGS(rs->op) & F_CTRL))
		{
			// Need to update the fetch history to the correct state
			bool br_taken = contexts[rs->context_id].pred->mypred->predict_brcond(rs->PC & 0x3ffff, MD_OP_FLAGS(rs->op));
			if (contexts[rs->context_id].pred->mypred && alg1 && brhistory && ((MD_OP_FLAGS(rs->op) & (F_CTRL)) == (F_CTRL))){
				contexts[rs->context_id].pred->mypred->FetchHistoryUpdate (rs->PC & 0x3ffff,MD_OP_FLAGS(rs->op), rs->next_PC != (rs->PC + sizeof(md_inst_t)), rs->next_PC & 0x7f);
			}
			if (!brupdate_cond || (MD_OP_FLAGS(rs->op) & F_COND)){
				if (!brupdate && alg1) {
					assert(contexts[rs->context_id].pred->mypred != NULL);
					contexts[rs->context_id].pred->mypred->update_brcond(rs->PC & 0x3ffff, MD_OP_FLAGS(rs->op), rs->next_PC != (rs->PC + sizeof(md_inst_t)), rs->next_PC & 0x7f);
				}
			}
		}
				...
				&& (MD_OP_FLAGS(rs->op) & F_CTRL))
		{
			bool correct_pred = (rs->pred_PC == rs->next_PC);
			if (correct_pred) {
				smtq[rs->context_id].clear_br(rs->seq, true);
			} else {
				if (rs->recover_inst)
					smtq[rs->context_id].clear_br(rs->seq);
			}
	...
	while ((rs = issue_exec_q_next_event()))
	{
		...
		if(rs->in_IQ == 1)
		{
			...
			if(rs->in_IQ){
				...
				rs->in_IQ = 0;
				contexts[rs->context_id].debug_count--;
				assert(contexts[rs->context_id].debug_count >= 0);
			}
		}
	}
}
							...
							if(rs->physreg >= 0){
								...
								if(!mystricmp(recovery_model, "squash")){
									...
									if(rs->addr){
										...
										assert(rs->addr);
										bpred_update(false, contexts[rs->context_id].load_lat_pred,
...
tracer_recover(int context_id)
{
	...
	contexts[context_id].debug_count -= contexts[context_id].fetch_num;
...
register_rename(void)
{
			...
			&& (include_spec || !contexts[disp_context_id].spec_mode))
	{
		...
		if((contexts[disp_context_id].fetch_num == 0)
				...
				|| ((smtq[disp_context_id].get_spec_depth()>smt_qos_th_spec_depth) && smt_qos_enable && smt_qos_enable_spec && smtq[disp_context_id].is_throttled())
			...
			for(j = 0; j<num_contexts; j++){
				bool smtq_ok = !((smtq[j].get_spec_depth()>smt_qos_th_spec_depth) && smt_qos_enable && smt_qos_enable_spec && smtq[j].is_throttled());
		...
		if(fetch_redirected[disp_context_id]){
			/* consume instruction from IFETCH -> RENAME queue */
			contexts[disp_context_id].debug_count--;
			assert(contexts[disp_context_id].debug_count >= 0);
...

		{
			...
			if(my_regs.dest != REG_NONE){
				if(find_free_physreg(my_regs.dest, disp_context_id) < 0){
...
#include "machine.def"
		...
		if (MD_OP_FLAGS(op) & F_CTRL) {
			sim_total_branches++;
			smtq_br_info sbi (rs->seq, 0);
			smtq[disp_context_id].spec_br.push_back(sbi);

		} else if (smtq[disp_context_id].spec_br.size()) {
			smtq[disp_context_id].spec_br.back().cnt++;
		}
		...
		if (!contexts[disp_context_id].spec_mode)
		{
	     ...
	     non-speculative state is committed into the BTB */
			if (MD_OP_FLAGS(op) & F_CTRL)
			{
				...
				{
					bpred_update(brupdate,contexts[disp_context_id].pred,
		...
		if (MD_OP_FLAGS(op) & F_CTRL)
		{
			bool br_taken = contexts[disp_context_id].pred->mypred->predict_brcond(regs->regs_PC & 0x3ffff, MD_OP_FLAGS(op));
			if (contexts[disp_context_id].pred->mypred && alg1 && !brhistory && ((MD_OP_FLAGS(op) & (F_CTRL)) == (F_CTRL))){
				contexts[disp_context_id].pred->mypred->FetchHistoryUpdate (regs->regs_PC & 0x3ffff,MD_OP_FLAGS(op),regs->regs_NPC != (regs->regs_PC + sizeof(md_inst_t)),regs->regs_NPC & 0x7f); //TODO:change in predictorcode for brtype DONE
			}
			if (!brupdate_cond || (MD_OP_FLAGS(op) & F_COND)){
				if (brupdate && alg1 ) {
				assert(contexts[disp_context_id].pred->mypred != NULL);

				contexts[disp_context_id].pred->mypred->update_brcond(regs->regs_PC & 0x3ffff, MD_OP_FLAGS(op), regs->regs_NPC != (regs->regs_PC + sizeof(md_inst_t)), regs->regs_NPC & 0x7f);
				}
			}
		}
	...
	for(unsigned int j = 0; j<num_contexts; j++){
		bool smtq_th = ((smtq[j].get_spec_depth()>smt_qos_th_spec_depth) && smt_qos_enable && smt_qos_enable_spec && smtq[j].is_throttled());
		if (smtq_th) {
			smtq[j].th_spec_cycles++;
		}
	}
...
dispatch(void)
{
	...
	while(n_dispatched < decode_width){
		...
		my_iq_num = alloc_iq_entry(disp_context_id);
		if(my_iq_num < 0){
		...
		rs->in_IQ = TRUE;
		assert(iq[my_iq_num].state != IQ_ENTRY_FREE);
...
fetch(int* current_fetch_context_ptr)
{
			...
			i++)
	{
		...
		contexts[current_fetch_context].debug_count++;
...
int my_comparator(const void * a1, const void * a2){
	return (contexts[*(int*)a1].debug_count - contexts[*(int*)a2].debug_count);
}
...
#include "machine.def"
				...
				else
				{
					if(addr){
						...
						if(!mystricmp(recovery_model, "squash")){
							...
							bpred_update(false,contexts[current_context].load_lat_pred,
									/* branch address */addr, //regs->regs_PC,
									/* actual target address */pred_hit_L1,
			...
			if (MD_OP_FLAGS(op) & F_CTRL){
				...
				bpred_update(false, contexts[current_context].pred,
						/* branch address */regs->regs_PC,
						/* actual target address */regs->regs_NPC,
						/* taken? */regs->regs_NPC != (regs->regs_PC +
								sizeof(md_inst_t)),
	...
	iq = new iq_entry_class[iq_size];

	for(i=0;i<iq_size;i++){
		iq[i].state = IQ_ENTRY_FREE;
	}
	...
	for (;;)
	{
		...
		//
		//
		if(!mystricmp(fetch_policy, "round_robin")){
...
void smt_print_stats(){
	...
	for(unsigned int i=0;i<num_contexts;i++){
		cout<<"IPC for thread " << i <<" = "<< contexts[i].sim_num_insn/(float)sim_cycle<<endl;
	}
	print_smtq_stats(num_contexts);
}
