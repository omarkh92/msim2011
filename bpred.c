...
bpred_create(bpred_class pclass,	/* type of predictor to create */
	     unsigned int bimod_size,	/* bimod table size */
	     unsigned int l1size,	/* 2lev l1 table size */
	     unsigned int l2size,	/* 2lev l2 table size */
	     unsigned int meta_size,	/* meta table size */
	     unsigned int shift_width,	/* history register width */
	     unsigned int hxor,  	/* history xor address flag */
	     unsigned int btb_sets,	/* number of sets in BTB */ 
	     unsigned int btb_assoc,	/* BTB associativity */
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  struct bpred_t *pred = new bpred_t();
  ...
  case BPredComb:
    ...
    /* 2-level component */
    pred->dirpred.twolev = 
      bpred_dir_create(BPred2Level, l1size, l2size, shift_width, hxor);
  ...
  case BPred2Level:
    pred->dirpred.twolev = 
      bpred_dir_create(pclass, l1size, l2size, shift_width, hxor);
...
bpred_dir_create (
  bpred_class pclass,	/* type of predictor to create */
  ...
  unsigned int hxor)	    	/* history xor address flag */
{
  ...
  default:
    panic("bogus branch direction predictor pclass");
  }
  ...
  FILE *stream)			/* output stream */
{
  ...
  case BPred2Level:
    fprintf(stream,
      ...
      pred_dir->config.two.hxor ? "" : "no", pred_dir->config.two.l2size);
    break;
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  default:
    panic("bogus branch predictor pclass");
  }
}
					 ...
					 * used on mispredict recovery */
{
  ...
  if (pred->mypred && alg1 && (MD_OP_FLAGS(op) & (F_CTRL|F_COND)) == (F_CTRL|F_COND)){
	  used = true;
	  gpred =pred->mypred->predict_brcond(baddr & 0x3ffff, MD_OP_FLAGS(op));
	  if (gpred){
		  //useMe=BPredTaken;
		  takenFlag=true;
	  } else{
  ...
  if (pbtb == NULL)
    {
	  ...
	  bool btaken = used ? gpred : (*(dir_update_ptr->pdir1) >= 2);
      return (btaken
	      ? /* taken */ 1
	      : /* not taken */ 0);
    }
...
void
bpred_update(bool retire,struct bpred_t *pred,	/* branch predictor instance */
	     md_addr_t baddr,		/* branch address */
